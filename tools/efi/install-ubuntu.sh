...
echo iomemory_vsl4 >> /etc/initramfs-tools/modules
cd /root/iomemory-vsl4
make dkms
release=$(uname -r)
nrelease=$(ls -1 /lib/modules | grep -v $release)
dkms_ver=$(dkms status | grep ^iomemory-vsl4 | awk -F', ' '{ print $2 }')
dkms build -m iomemory-vsl4 -v $dkms_ver -k $nrelease
dkms install -m iomemory-vsl4 -v $dkms_ver -k $nrelease
EOT
